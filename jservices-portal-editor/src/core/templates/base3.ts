  ...
  <main class="sky-shell">
    ...
    <% if (plans.length) { %>
      ...
      <% plans.forEach(function(plan) { %>
        <a href="<%= (features && features.enablePaymentLinks) ? (buildTiketMomoPaymentUrl(plan) || '#') : '#' %>" class="sky-plan-card">
          <div style="flex: 1;">
            <span class="sky-plan-name"><%= plan.displayName %></span>
            <span class="sky-plan-meta"><%= plan.durationLabel %> • <%= plan.speedLabel %></span>
            <% if (features && features.enablePaymentLinks && (payment.aggregator !== 'none' || buildTiketMomoPaymentUrl(plan))) { %>
